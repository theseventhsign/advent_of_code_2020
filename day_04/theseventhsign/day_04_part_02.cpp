...
IsValid(passport *Passport)
{
		...
		++FieldIndex)
	{
		...
		if(strcmp(Field->Key, "byr") == 0)
		{
			...
			{
				int BirthYear = atoi(Field->Value);
		...
		else if(strcmp(Field->Key, "iyr") == 0)
		{
			if(strlen(Field->Value) == 4)
			{
				int IssueYear = atoi(Field->Value);
		...
		else if(strcmp(Field->Key, "eyr") == 0)
		{
			if(strlen(Field->Value) == 4)
			{
				int ExpirationYear = atoi(Field->Value);
		...
		else if(strcmp(Field->Key, "hgt") == 0)
		{
			...
			char FirstChar;
			char SecondChar;
			if(sscanf(Field->Value, "%d%c%c", &Height, &FirstChar, &SecondChar) == 3)
			{
		...
		else if(strcmp(Field->Key, "hcl") == 0)
		{
			...
			int KeyLength = strlen(Field->Value);
			if((KeyLength == 7) &&
			   ((At - Field->Value) == KeyLength))
			{
				FieldCounts[4]++;
			}
		}
		...
		{
			if((strcmp(Field->Value, "amb") == 0) ||
			   (strcmp(Field->Value, "blu") == 0) ||
			   (strcmp(Field->Value, "brn") == 0) ||
			   (strcmp(Field->Value, "gry") == 0) ||
			   (strcmp(Field->Value, "grn") == 0) ||
			   (strcmp(Field->Value, "hzl") == 0) ||
			   (strcmp(Field->Value, "oth") == 0))
			{
				FieldCounts[5]++;
			}
		}
		...
		{
			if(strlen(Field->Value) == 9)
			{
				b32 IsNumber = true;
				for(char *At = Field->Value;
					*At;
					At++)
				{
					if(!isdigit(*At))
