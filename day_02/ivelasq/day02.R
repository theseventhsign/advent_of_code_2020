...
input2 <-
  input %>% 
  mutate(first_num = as.numeric(stringr::str_extract(passwords, "[^-]+")), # before dash
         second_num = as.numeric(stringr::str_extract(passwords, "(?<=-).*?(?=\\s)")), # after dash and before space
         letter = str_trim(stringr::str_extract(passwords, " .")), # after space
         pw = str_trim(stringr::str_extract(passwords, "(?<=:).*")), # after colon
         occurrences = stringr::str_count(pw, letter),
         valid = case_when(
           occurrences <= second_num & occurrences >= first_num ~ "valid",
           TRUE ~ "not valid"
         ))
...
# input3 <-
#   input2 %>% 
#   mutate(length = str_length(pw),
#          long = case_when(length < second_num ~ "too long",
#                           TRUE ~ "ok"))

# length(unique(input3$pw))
...
input3 <-
  ...
  mutate(pos = list(
    data.frame(
      str_locate_all(string = pw,
                     pattern = letter)))) %>% 
  unnest(pos) %>% 
  mutate(match_num = case_when(
    first_num == start | second_num == start ~ 1,
    TRUE ~ 0))

input3 %>%
  group_by(passwords, first_num, second_num, letter, pw) %>% 
  summarise(sum = sum(match_num)) %>% 
  ungroup() %>% 
  count(sum)